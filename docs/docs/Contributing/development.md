...
- **Add a dependency** — edit `pyproject.toml`, then `uv lock && uv sync`
- **Install from the lockfile** — `uv sync`
- The `requirements.txt` files under `demo/`, `deploy_eks/`, and `uaef-service/`
  belong to those standalone apps. They are not used for the core `uaef`
  package.

### Security constraints

`pyproject.toml` has a `# Security constraints` block where dependency floors
...
Line length is 100 (see `[tool.black]` and `[tool.ruff]` in `pyproject.toml`).
